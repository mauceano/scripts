...
# OLD $searchresults = api get "/searchvms?environment=SQL&entityTypes=kSQL&entityTypes=kVMware&vmName=$sourceDB"
$searchresults = api get -v2 "data-protect/search/objects?sourceIds=$ID&isProtected=true&environments=kSQL&searchString=$sourceDB"

# narrow the search results to the correct source server
$dbresults = $searchresults.objects | Where-Object {$_.sourceInfo.name -eq $sourceServer } | `
                                  Where-Object {$_.name.Split('/')[1] -eq $sourceDB } | `
                                  Where-Object {$_.name -in $sourceDBObjectNames}
