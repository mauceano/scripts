...
# Check if Databases are Standalone or AAG, and assign accordingly:
$typeEsc = "type"
$dbType = ($dbresults.vms.vmDocument.objectId.entity.sqlEntity).$typeEsc
if($dbType -eq "1") {
	# filter by source instance
	if($sourceInstance){
		$dbresults.vms = $dbresults.vms | Where-Object {$_.vmDocument.objectId.entity.sqlEntity.instanceName -eq $sourceInstance}
		if(! $dbresults.vms){
...
if($dbType  -eq "5") {
	$aagNodes = @($dbresults.vms.vmDocument.objectId.entity.sqlEntity.aagDbCopyInfoVec.hostEntityId | Sort-Object -Unique)
	if($sourceInstance){
		$dbresults.vms = $dbresults.vms | Where-Object {$_.vmDocument.entity.aagDbCopyInfoVec.instanceName -eq $sourceInstance}
		if(! $dbresults.vms){
			Write-Host "no DBs found for $sourceServer/$sourceInstance" -ForegroundColor Yellow
			exit
		}
...
foreach($sourceDbName in $sourceDbNames | Sort-Object){
    ...
    Write-Host "`n"$sourceServer" - $($search.objects[0].name)"
